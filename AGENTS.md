...
- For internal custom primitive extensions, prefer
  ...
  keep extension-seam callback composition ordering regression coverage in
  `tests/unit/test_interaction.cpp` when layering extra low-level callbacks onto extension nodes,
  keep extension-seam routed reentrancy suppression regression coverage in
  `tests/unit/test_interaction.cpp` when extension callback handlers can invoke node callbacks,
  keep extension-seam NodeCallbackHandle interop restoration coverage in
  `tests/unit/test_interaction.cpp` when callback-table override/restore paths are touched,
  keep extension-seam NodeCallbackHandle move/destroy regression coverage in
  `tests/unit/test_interaction.cpp` when callback-handle ownership/lifetime paths are changed,
  keep extension-seam visibility-toggle callback lifecycle coverage in
  `tests/unit/test_interaction.cpp` when visibility state can change post-build,
  keep extension-seam hit-test-toggle callback lifecycle coverage in
  ...
  keep extension-seam non-focusable focus/blur callback wiring coverage in
  `tests/unit/test_interaction.cpp` when extension callbacks are provided for non-focusable nodes
  to ensure focus/blur handlers stay unwired,
...
- Keep input/focus and widget-spec sanitization fuzz/property tests deterministic
  (fixed RNG seeds + explicit regression corpus) so failures are reproducible;
  maintain the sanitization corpus in `tests/unit/test_spec_validation.cpp`.
...
- Keep accessibility role/state defaults aligned with
  `docs/accessibility-semantics-roadmap.md`; when semantic mappings change,
  update deterministic snapshot/export coverage in
  `tests/unit/test_spec_validation.cpp` in the same change.
