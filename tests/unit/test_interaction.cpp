...
TEST_CASE("PrimeStage internal extension primitive seam supports non-focusable pointer interactivity without keyboard routing") {
  PrimeFrame::Frame frame;
  ...
  PrimeFrame::Primitive const* extensionRect =
      findRectPrimitiveByTokenInSubtree(frame, extension.nodeId(), 993u);
...
TEST_CASE("PrimeStage internal extension primitive seam preserves focus callback lifecycle across visibility toggles") {
  PrimeFrame::Frame frame;
  PrimeStage::UiNode root = createRoot(frame, 340.0f, 220.0f);
  ...
  PrimeFrame::Primitive const* firstRect =
      findRectPrimitiveByTokenInSubtree(frame, first.nodeId(), 985u);
  PrimeFrame::Primitive const* secondRect =
      findRectPrimitiveByTokenInSubtree(frame, second.nodeId(), 986u);
  REQUIRE(firstRect != nullptr);
  REQUIRE(secondRect != nullptr);
}

TEST_CASE("PrimeStage internal extension primitive seam preserves focus callback lifecycle across hit-test toggles") {
  PrimeFrame::Frame frame;
  PrimeStage::UiNode root = createRoot(frame, 340.0f, 220.0f);
  ...
  PrimeFrame::Node const* firstNode = frame.getNode(first.nodeId());
  REQUIRE(firstNode != nullptr);
  PrimeFrame::CallbackId firstCallbackId = firstNode->callbacks;
  REQUIRE(firstCallbackId != PrimeFrame::InvalidCallbackId);
  ...
  CHECK(firstNode->callbacks == firstCallbackId);
  ...
  CHECK(firstNode->callbacks == firstCallbackId);
  int focusCountBeforeRefocus = firstFocusCount;
  clickCenter(first.nodeId(), 3);
  CHECK(focus.focusedNode() == first.nodeId());
  CHECK(firstFocusCount == focusCountBeforeRefocus + 1);

  PrimeFrame::Primitive const* firstRect =
      findRectPrimitiveByTokenInSubtree(frame, first.nodeId(), 989u);
  PrimeFrame::Primitive const* secondRect =
      findRectPrimitiveByTokenInSubtree(frame, second.nodeId(), 990u);
  REQUIRE(firstRect != nullptr);
  REQUIRE(secondRect != nullptr);
}
