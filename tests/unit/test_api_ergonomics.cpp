...
TEST_CASE("PrimeStage examples stay split between canonical and advanced tiers") {
  ...
  CHECK(advancedEscape.find("Treat escape-hatch additions as docs+tests-coupled changes: update docs and") !=
        std::string::npos);
  CHECK(advancedEscape.find("In checklist updates, keep canonical-first fallback explicit:") !=
        std::string::npos);
  CHECK(advancedEscape.find("docs/advanced-escape-hatches.md") != std::string::npos);
...
TEST_CASE("PrimeStage internal custom-primitive extension seam stays typed and constrained") {
  ...
  CHECK(interaction.find("internal extension primitive seam restores callbacks after NodeCallbackHandle override") !=
        std::string::npos);
  CHECK(interaction.find("internal extension primitive seam tolerates NodeCallbackHandle move and destroyed node reset") !=
        std::string::npos);
  ...
  CHECK(agents.find("extension-seam NodeCallbackHandle interop restoration coverage") !=
        std::string::npos);
  CHECK(agents.find("extension-seam NodeCallbackHandle move/destroy regression coverage") !=
        std::string::npos);
  ...
  CHECK(agents.find("extension-seam focus-callback lifecycle coverage across hit-test toggles") !=
        std::string::npos);
  CHECK(agents.find("extension-seam disabled/hidden runtime-gating lifecycle coverage under post-build toggles") !=
        std::string::npos);
...
TEST_CASE("PrimeStage onboarding docs separate canonical and advanced usage") {
  ...
  CHECK(advanced.find("Treat escape-hatch additions as docs+tests-coupled changes: update docs and") !=
        std::string::npos);
  CHECK(advanced.find("In checklist updates, keep canonical-first fallback explicit:") !=
        std::string::npos);
  CHECK(advanced.find("docs/advanced-escape-hatches.md") != std::string::npos);
...
TEST_CASE("PrimeStage default behavior matrix is documented and enforced") {
  ...
  std::filesystem::path progressCppPath = repoRoot / "src" / "PrimeStageProgress.cpp";
  std::filesystem::path paragraphCppPath = repoRoot / "src" / "PrimeStageParagraph.cpp";
  std::filesystem::path tabsCppPath = repoRoot / "src" / "PrimeStageTabs.cpp";
  std::filesystem::path textSelectionOverlayCppPath = repoRoot / "src" / "PrimeStageTextSelectionOverlay.cpp";
  ...
  REQUIRE(std::filesystem::exists(progressCppPath));
  REQUIRE(std::filesystem::exists(paragraphCppPath));
  REQUIRE(std::filesystem::exists(tabsCppPath));
  REQUIRE(std::filesystem::exists(textSelectionOverlayCppPath));
  ...
  std::string containersCpp((std::istreambuf_iterator<char>(containersInput)),
                            std::istreambuf_iterator<char>());
  REQUIRE(!containersCpp.empty());
  std::ifstream progressInput(progressCppPath);
  REQUIRE(progressInput.good());
  std::string progressCpp((std::istreambuf_iterator<char>(progressInput)),
                          std::istreambuf_iterator<char>());
  REQUIRE(!progressCpp.empty());
  std::ifstream paragraphInput(paragraphCppPath);
  REQUIRE(paragraphInput.good());
  std::string paragraphCpp((std::istreambuf_iterator<char>(paragraphInput)),
                           std::istreambuf_iterator<char>());
  REQUIRE(!paragraphCpp.empty());
  ...
  std::ifstream textSelectionOverlayInput(textSelectionOverlayCppPath);
  REQUIRE(textSelectionOverlayInput.good());
  std::string textSelectionOverlayCpp((std::istreambuf_iterator<char>(textSelectionOverlayInput)),
                                      std::istreambuf_iterator<char>());
  REQUIRE(!textSelectionOverlayCpp.empty());
  std::ifstream textLineInput(textLineCppPath);
  REQUIRE(textLineInput.good());
  std::string textLineCpp((std::istreambuf_iterator<char>(textLineInput)),
                          std::istreambuf_iterator<char>());
...
TEST_CASE("PrimeStage collection entrypoints are split into dedicated translation units") {
  ...
  CHECK(core.find("UiNode UiNode::createParagraph(ParagraphSpec const& specInput)") == std::string::npos);
  CHECK(core.find("UiNode UiNode::createParagraph(std::string_view text,") == std::string::npos);
  CHECK(core.find("UiNode UiNode::createVerticalStack(StackSpec const& spec)") == std::string::npos);
  CHECK(core.find("UiNode UiNode::createHorizontalStack(StackSpec const& spec)") == std::string::npos);
  CHECK(core.find("UiNode UiNode::createOverlay(StackSpec const& spec)") == std::string::npos);
  CHECK(core.find("UiNode UiNode::createPanel(PanelSpec const& specInput)") == std::string::npos);
  CHECK(core.find("UiNode UiNode::createPanel(PrimeFrame::RectStyleToken rectStyle, SizeSpec const& size)") ==
        std::string::npos);
  ...
  CHECK(core.find("PanelSpec normalizePanelSpec(PanelSpec const& specInput)") !=
        std::string::npos);
  CHECK(core.find("TextSelectionOverlaySpec normalizeTextSelectionOverlaySpec(TextSelectionOverlaySpec const& specInput)") !=
        std::string::npos);
  ...
  REQUIRE(textLineInput.good());
  std::string textLineSource((std::istreambuf_iterator<char>(textLineInput)),
                             std::istreambuf_iterator<char>());
  REQUIRE(!textLineSource.empty());
  CHECK(textLineSource.find("UiNode UiNode::createTextLine(TextLineSpec const& specInput)") !=
        std::string::npos);
  ...
  REQUIRE(paragraphInput.good());
  std::string paragraphSource((std::istreambuf_iterator<char>(paragraphInput)),
                              std::istreambuf_iterator<char>());
  REQUIRE(!paragraphSource.empty());
  CHECK(paragraphSource.find("UiNode UiNode::createParagraph(ParagraphSpec const& specInput)") !=
        std::string::npos);
  ...
  CHECK(textFieldSource.find("Internal::normalizeTextFieldSpec(specInput)") !=
        std::string::npos);
  CHECK(textFieldSource.find("Internal::makeWidgetRuntimeContext(") !=
        std::string::npos);
  CHECK(textFieldSource.find("Internal::attachFocusOverlay(runtime,") !=
        std::string::npos);
  CHECK(textFieldSource.find("Internal::addDisabledScrimOverlay(runtime,") !=
        std::string::npos);
  ...
  CHECK(selectableTextSource.find("Internal::normalizeSelectableTextSpec(specInput)") !=
        std::string::npos);
  CHECK(selectableTextSource.find("Internal::makeWidgetRuntimeContext(") !=
        std::string::npos);
  CHECK(selectableTextSource.find("Internal::attachFocusOverlay(runtime,") !=
        std::string::npos);
  CHECK(selectableTextSource.find("Internal::addDisabledScrimOverlay(runtime,") !=
        std::string::npos);

  std::ifstream containersInput(containersPath);
  REQUIRE(containersInput.good());
  std::string containersSource((std::istreambuf_iterator<char>(containersInput)),
                               std::istreambuf_iterator<char>());
  REQUIRE(!containersSource.empty());
  CHECK(containersSource.find("UiNode UiNode::createVerticalStack(StackSpec const& spec)") !=
        std::string::npos);
  CHECK(containersSource.find("UiNode UiNode::createHorizontalStack(StackSpec const& spec)") !=
        std::string::npos);
  CHECK(containersSource.find("UiNode UiNode::createOverlay(StackSpec const& spec)") !=
        std::string::npos);
  CHECK(containersSource.find("UiNode UiNode::createPanel(PanelSpec const& specInput)") !=
        std::string::npos);
  ...
  REQUIRE(textSelectionOverlayInput.good());
  std::string textSelectionOverlaySource((std::istreambuf_iterator<char>(textSelectionOverlayInput)),
                                         std::istreambuf_iterator<char>());
  REQUIRE(!textSelectionOverlaySource.empty());
  CHECK(textSelectionOverlaySource.find("UiNode UiNode::createTextSelectionOverlay(TextSelectionOverlaySpec const& specInput)") !=
        std::string::npos);
  ...
  CHECK(collections.find("Internal::makeParentNode(runtime)") != std::string::npos);
  ...
  std::string tabs((std::istreambuf_iterator<char>(tabsInput)),
                   std::istreambuf_iterator<char>());
  REQUIRE(!tabs.empty());
  CHECK(tabs.find("UiNode UiNode::createTabs(TabsSpec const& specInput)") != std::string::npos);
